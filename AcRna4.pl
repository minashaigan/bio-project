...
## input : RNA sequence input
	open (my $in,"RnaSeq3.txt") or die "Can't read $!";  
	my $Rna = <$in>;
	$Rna = lc($Rna);
	$Rna=~ s/\R//g;
	my $Structure = <$in>;
	...
	while($terminate == 0 ){
		...
		while($kk<=$num_ant){
			...
			if($energy<0 and $energy<$min_energy-$min_energy*0.1){
				$min_energy = $energy;
...
sub Initial_pheremones {
	for(my $i=0;$i<scalar(@stems);$i++){
		...
		for(my $j=0;$j<scalar(@stems);$j++){
			if( ($stems[$i]{u1}+$stems[$i]{u3}<=$stems[$j]{u1} and $stems[$j]{u2}<=$stems[$i]{u2}-$stems[$i]{u3} ) or ( $stems[$i]{u2}<$stems[$j]{u1} ) ){
				$num++;
			}
		}
		#print $i,"= ",$num,"\n";
...
sub SelectNextStem{
	while(@allowed){
		...
		my $rdm = rand(1);
			...
			my $flag = 0;
			for(my $j=0;$j<scalar(@allowed);$j++){
				$cur_prob += $pheremones[$solution[$i]][$allowed[$j]]*$pheremones[$solution[$i]][$allowed[$j]]*$heur_info[$solution[$i]][$allowed[$j]]/$sum;
				if($rdm<$cur_prob){
		...
		for(my $j=0;$j<scalar(@{$ants_solution[$i]});$j++){
			for(my $k=$j+1;$k<scalar(@{$ants_solution[$i]});$k++){
				if($energy<0 and $min_energy<0){
					...
					$pheremones[$ants_solution[$i][$j]][$ants_solution[$i][$k]] += 0;
					$pheremones[$ants_solution[$i][$k]][$ants_solution[$i][$j]] += 0;
				}
